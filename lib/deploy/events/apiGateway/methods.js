    ...
    const integration = {
      ...
      RequestTemplates: {
        'application/json': {
          'Fn::Join': [
            '', [
              "#set( $action = $util.escapeJavaScript($input.json('$.action')).replaceAll(\"\\\\'\",\"\'\") ) \n\n",
              "#set( $ref = $util.escapeJavaScript($input.json('$.ref')).replaceAll(\"\\\\'\",\"\'\") ) \n\n",
              "#set( $pull_request = $util.escapeJavaScript($input.json('$.pull_request')).replaceAll(\"\\\\'\",\"\'\") ) \n\n",
              '{"input": \"{\\"body\\": {\\"action\\": $action, \\"ref\\": $ref, \\"pull_request\\": $pull_request}, \\"headers\\": {\\"X-Hub-Signature\\": \\"$input.params(\'X-Hub-Signature\')\\", \\"X-GitHub-Event\\":\\"$input.params(\'X-GitHub-Event\')\\", \\"X-GitHub-Delivery\\":\\"$input.params(\'X-GitHub-Delivery\')\\"}}\","name": "$context.requestId","stateMachineArn":"',
              {
